// Shared normalization for LLM-returned item refs (C<digits> cluster,
// S<digits> singleton, T<digits> thread).
...
const REF_RE = /[cst]\d+/i;
const REF_RE_GLOBAL = /[cst]\d+/gi;

/** First C/S/T ref in a token, upper-cased; null if none present. */
export function normalizeRef(token: string): string | null {
  const m = token.match(REF_RE);
  return m ? m[0].toUpperCase() : null;
}

/** All C/S/T refs in a string, in order, upper-cased. */
